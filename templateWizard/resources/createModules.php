<?php
	...
	foreach ($modules as $value) {
	    ...
	    $newModuleID = createModule($courseID, $moduleParams, $tokenHeader);
	    ...
	    if ($primaryTemplate == "true"){
	    	...
	    	$pageBody = getPageBody($courseID, 'primary-template', $tokenHeader);
	    	// Replace the existing module header details with content from form
			// var_dump($pageBody);
	    	// Find and replace module prefix
	    	$replacePrefix = get_string_between($pageBody, 'class="mod-text">', '</span>');
	    	// var_dump($replacePrefix);
	    	$explodedPrefix = explode(" ", $moduleTitlePrefix);
	    	$prefixText = $explodedPrefix[0];
			$pageBody = str_replace('class="mod-text">'.$replacePrefix.'</span>', 'class="mod-text">'.$prefixText.' </span>', $pageBody);
	    	// Find and replace module number
	    	$replaceNumber = get_string_between($pageBody, 'class="mod-num">', '</span>');
	    	// var_dump($replaceNumber);
			$pageBody = str_replace('class="mod-num">'.$replaceNumber.'</span>', 'class="mod-num">'.$moduleNumber.'</span>', $pageBody);
	    	...
	    	$replaceTitle = get_string_between($pageBody, 'id="banner-right">', '</span>');
	    	// var_dump($replaceTitle);
			$pageBody = str_replace('id="banner-right">'.$replaceTitle.'</span>', 'id="banner-right">'.$titleText.'</span>', $pageBody);
	    	// var_dump($pageBody);


			$pageParams = 'wiki_page[title]='.$pageTitle.'&wiki_page[body]='.urlencode($pageBody);
			$newPage = createPage($courseID, $pageParams, $tokenHeader);
			...
			$modulePage = createModuleItem($courseID, $newModuleID, $itemParams, $tokenHeader);
	    }
	    ...
	    if ($secondaryTemplateCount != ""){
	    	$pageBody = getPageBody($courseID, 'secondary-template', $tokenHeader);
			// Find and replace module prefix
			$replacePrefix = get_string_between($pageBody, 'class="mod-text">', '</span>');
			// var_dump($replacePrefix);
			$explodedPrefix = explode(" ", $moduleTitlePrefix);
			$prefixText = $explodedPrefix[0];
	  	   ...
	  	   for ($i=1; $i<=$secondaryTemplateCount; $i++){
				...
				$replaceNumber = get_string_between($pageBody, 'class="mod-num">', '</span>');
				// var_dump($replaceNumber);
				$pageBody = str_replace('class="mod-num">'.$replaceNumber.'</span>', 'class="mod-num">'.$moduleNumber.'.'.$i.'</span>', $pageBody);
	    ...
	    for ($i=1; $i<=$assignmentCount; $i++){
		    ...
		    $assignmentID = createGenericAssignment($courseID, $assignmentParams, $tokenHeader);
			...
			$moduleAssignment = createModuleItem($courseID, $newModuleID, $itemParams, $tokenHeader);
		}
	    ...
	    for ($i=1; $i<=$discussionCount; $i++){
		    ...
		    $discussionID = createGenericDiscussion($courseID, $discussionParams, $tokenHeader);
			...
			$moduleDiscussion = createModuleItem($courseID, $newModuleID, $itemParams, $tokenHeader);
		}
	    ...
	    for ($i=1; $i<=$quizCount; $i++){
		    ...
		    $quizID = createGenericquiz($courseID, $quizParams, $tokenHeader);
		    $itemParams = 'module_item[title]='.urlencode($moduleTitlePrefix.' Quiz '.$i).'&module_item[type]=quiz&module_item[content_id]='.$quizID;
			$modulequiz = createModuleItem($courseID, $newModuleID, $itemParams, $tokenHeader);
		}
	}
...
<head>
	...
	<title>BlueLine Template Wizard - Front Page</title>
