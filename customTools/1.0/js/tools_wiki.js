...
// include is found in canvasGlobal.js
	...
	(function() {
        function editorExistenceCheck() {
            ...
            if ($("#wiki_page_body_ifr").contents().find("#tinymce").length > 0) {
                editorExists = true;
            }
		...
		$("#wiki_page_submit").click(function (e){
			// e.preventDefault();
			$contents = $(iframeID).contents();
			// Make sure the accordion h3 titles are wrapped in <a href="#">
			if($contents.find(".custom-accordion h3")){
				$(iframeID).contents().find(".custom-accordion h3").each(function(){
					$(this).contents().wrap('<a href="#" />');
				});
			}
			if($contents.find(".custom-tabs .tab-list li")){
				$contents.find(".custom-tabs .tab-list li").each(function(){
					var myClass = $(this).attr('class');
					var myClass = myClass.replace(" current", "");
					$(this).contents().wrap('<a href="#'+myClass+'" />');
				});
			}
			// Handle when a box was unchecked but the remove button wasn't clicked
			$contents.find(".toRemove").removeClass("toRemove");
			// If the current theme doesn't support bottom-banner, remove it
			var templateClass = $("#wiki_page_body_ifr").contents().find("#template-wrapper").attr("class");
			if($.inArray(templateClass, bottomBannerThemeArray) == -1){
				$("#wiki_page_body_ifr").contents().find('#banner-bottom').attr("data-mce-style", "").remove();
			}
		});
		...
		////// WIKI-PAGE //////
			function setupWikiPage(){
				...
				// Add the tools section in the right panel if it doesn't exist
					if(!$("#custom-tools-accordion").length>0){
						var mceInstance = "wiki_page_body";
						createToolsWrapper();
			...
			if($("#edit_assignment_wrapper").length>0){
				if(!$("#custom-tools-accordion").length>0){
					...
					// Add tools for custom-tools-accordion
						toolsOverview();
						studentVerification();
						contentIcons('assignment_description');
					// activate the accordion
					initializeToolsAccordion();
		...
		function accordionTabsTool(mceInstance){
			var addAccordionSection = '<h3>\
				...
				<div>\
					...
					<p style="margin-top: 10px;"><small><em>Use &ldquo;<a href="#" class="activateSections">Sections</a>&rdquo; tool to adjust position of accordion or tabs section on page.</em></small></p>\
				</div>';
			$("#custom-tools-accordion").append(addAccordionSection);
			...
			$(iframeID).contents().find(".custom-tabs .tab-list li").each(function(){
				$(this).find("a").contents().unwrap();
			});
			getAccPanels();
			getTabPanels();
			...
			//// TABS ////
				...
				// Tab style
					$(".tabMinimal").click(function (e){
						e.preventDefault();
						$(iframeID).contents().find(".custom-tabs").addClass("ui-tabs-minimal");
						$(this).addClass("active");
						$(".tabRegular").removeClass("active");
					});
					$(".tabRegular").click(function (e){
						e.preventDefault();
						$(iframeID).contents().find(".custom-tabs").removeClass("ui-tabs-minimal");
						$(this).addClass("active");
						$(".tabMinimal").removeClass("active");
					});
				// // Module List
				// 	$(".insertModuleTabs").click(function (e){
				// 		e.preventDefault();
				// 		tabsCheck();
				// 		insertModuleTabs();
	...
	////// Supporting functions  //////
		// Accordion //
			function accordionCheck(){
				if(!$(iframeID).contents().find('.custom-accordion').length>0){
					...
					// Insert the new section into the TinyMCE editor
						$(iframeID).contents().find("#template-content").append('<div class="custom-accordion" />');
					// Create an <li> for this section in the Sections List
						var newSectionControls = '<li>\
							...
							<label><input type="checkbox" class="customSection" checked value="'+newPanelClass+'">\
								<span class="sectionTitle">'+newPanelName+'</span>\
							</label>\
							</li>';
				...
				// Grab name from text field
					...
					// Create a new class using the section name
						if($(iframeID).contents().find(".custom-accordion h3").length>0){
							var panelCount = $(iframeID).contents().find(".custom-accordion h3").length;
							var panelNum = panelCount+1;
						} else {
							var panelNum = 0;
						}
					...
					// Insert the new section into the TinyMCE editor
						var newPanel = '<h3 class="'+newPanelClass+'">'+newAccPanelName+'</h3>\
							<div class="'+newPanelClass+'">\
								<p>'+newAccPanelName+' panel contents.</p>\
							</div>';
						$(iframeID).contents().find(".custom-accordion").append(newPanel);
					// Create an <li> for this section in the Sections List
						$("#accordionPanels").append('<li rel=".'+newPanelClass+'"><span title="Drag to reorder" class="move_item_link"><img alt="Move" src="/images/move.png?1366214258"></span>'+newAccPanelName+'\
							<a class="deleteAccPanel remove icon-end pull-right" rel=".'+newPanelClass+'" href="#" data-tooltip="left" title="Delete this panel.">\
			...
			function getAccPanels(){
				...
				if($contents.find(".custom-accordion").length>0){
					$contents.find(".custom-accordion h3").each(function(i){
					...
					$contents.find(".custom-accordion div").each(function(i){
						$(this).attr('class', 'panel'+i);
					});
					$(".deleteAccTool").show();
				} else {
					...
					$( "#accordionPanels" ).sortable({
						update: function( event, ui ) {
							...
							$('#accordionPanels li').each(function (i) {
								...
								$contents.find(".custom-accordion").append($contents.find('h3'+panelClass));
								$contents.find(".custom-accordion").append($contents.find('div'+panelClass));
							});
						}
					});
		...
		// Tabs //
				...
				if(!$(iframeID).contents().find('.custom-tabs').length>0){
					...
					// Insert the new section into the TinyMCE editor
						if($(".tabMinimal").hasClass("active")){
							$(iframeID).contents().find("#template-content").append('<div class="custom-tabs ui-tabs-minimal"><ul class="tab-list"></ul></div>');
						} else {
							$(iframeID).contents().find("#template-content").append('<div class="custom-tabs"><ul class="tab-list"></ul></div>');
						}
				}
			}
			function addTabPanel(){
				...
				// Grab name from text field
					...
					// Create a new class using the tab li
						if($(iframeID).contents().find(".custom-tabs .tab-list li").length>0){
							var panelCount = $(iframeID).contents().find(".tab-list li").length;
							var panelNum = panelCount;
						} else {
							var panelNum = 0;
						}

						var newPanelClass = "tabs-"+panelNum;
					// Insert the new section into the TinyMCE editor
						var newPanelLi = '<li class="'+newPanelClass+'">'+newTabPanelName+'</li>';
						var newPanelDiv = '<div id="tabs-'+panelNum+'" class="tab-content '+newPanelClass+'">\
								<p>'+newTabPanelName+' panel contents.</p>\
							</div>';
						$(iframeID).contents().find(".tab-list").append(newPanelLi);
						$(iframeID).contents().find(".custom-tabs").append(newPanelDiv);
					// Create an <li> for this section in the Sections List
						$("#tabPanels").append('<li rel=".'+newPanelClass+'"><span title="Drag to reorder" class="move_item_link"><img alt="Move" src="/images/move.png?1366214258"></span>'+newTabPanelName+'\
					...
					$("#tabPanels").sortable({
						update: function( event, ui ) {
							...
							$('#tabPanels li').each(function (i) {
								var panelClass = $(this).attr('rel');
								$contents.find(".tab-list").append($contents.find('li'+panelClass));
								$contents.find(".custom-tabs").append($contents.find('div'+panelClass));
							});
						}
					});
			...
			function deleteTabSection(){
				$(".deleteTabTool").click(function (e){
					e.preventDefault();
					$(iframeID).contents().find(".custom-tabs").remove();
					$("#tabPanels").empty();
					$(".deleteTabTool").hide();
					$(".tabsOptions form").show();
					// if($(".insertModuleTabs").length>0){
					// 	$(".insertModuleTabs").html('<i class="icon-add"></i> Add Module Tabs')
					// }
				});
			}
			...
			function getTabPanels(){
				$("#tabPanels").html("");
				var sortableTabs = true;
				var $contents = $(iframeID).contents();
				if($contents.find(".custom-tabs").length>0){
					$contents.find(".tab-list li").each(function(i){
						...
						if($contents.find("#moduleTabs").length>0){
							var connectedSectionIndex = $(this).index();
							var panelTitle = $contents.find(".tab-content").eq(connectedSectionIndex).find("a").text();
							if (panelTitle.length > 25){
								panelTitle = panelTitle.substring(0,20)+"...";
							}
							sortableTabs = false;
							sortableImage = '';
							$(".tabsOptions form").hide();
					...
					$contents.find(".custom-tabs div").each(function(i){
						if($(this).hasClass("current")){
					...
					$(".deleteTabTool").show();
					if($(iframeID).contents().find(".custom-tabs").hasClass("ui-tabs-minimal")){
			...
			// 			$(this).attr({
			// 				"id": "tab-"+i,
			// 				"class": "tab-content tab-"+i
			// 			}).html('<a href="'+moduleUrl+'" id="'+moduleID+'" class="connectedModule">'+moduleTitle+'</a>');
			// 			if (moduleTitle.length > 25){
			// 				var moduleTitle = moduleTitle.substring(0,20)+"...";
			// 			}
			// 			$("#tabPanels").append('<li>\
			// 				<a href="#" class="mt-markCurrent" rel="tab-'+i+'" title="Mark as current week"><i class="icon-check"></i><span class="screenreader-only">Mark as Current Week</span></a>\
			// 				'+moduleTitle+'\
			// 				</li>');
			// 		});
			// 		$contents.find(".custom-tabs").attr("id", "moduleTabs");
			// 		$contents.find("#context_modules").contents().unwrap();
			// 		$(".insertModuleTabs").html('<i class="icon-refresh"></i> Update Module List')
			// 		markCurrent();
			// 	});
			// }
			function markCurrent(){
				$(".mt-markCurrent").click(function (e){
					...
					$("#wiki_page_body_ifr").contents().find(".current").removeClass("current");
					$("#wiki_page_body_ifr").contents().find("."+tabNum).addClass("current");
					$("#wiki_page_body_ifr").contents().find("#"+tabNum).addClass("current");
...
///////////////////////////////
	...
	////// Custom Tools Accordion tab setup  //////
		function contentIcons(mceInstance){
			...
			/// ICONS ////
                ...
                // Trigger for Icon dialog
                    ...
                    // 	var arr = jQuery.grep(iconSearchArray, function (value) {
                    // 		var searchString = $("#searchIcons").val();
                    // 		var regExpMatch = "/\b"+searchString+"\S+\s?/g";

                    // 		console.log(searchString);
                    // 		search = new RegExp(searchString, "gi");
        ...
        //             autocomplete({
				    // 	source: iconSearchArray
				    // }).data("autocomplete")._renderItem = function (ul, item) {
				    //      return $("<li></li>")
				    //          .data("item.autocomplete", item)
				    //          .append('<a class="changeIcon" rel="' + item.label + '"><i class="' + item.label + '"></i></a>')
				    //          .appendTo(ul);
				    //  };
		}
		...
		function popupReady(mceInstance){
			...
			// Tooltips
				$(".addTooltip").click(function (e){
					...
					$contents.find(".createTooltip").prepend(toolTipElement).removeClass("createTooltip");
					checkTooltips();
				});
			...
			// Popovers
				$(".addPopover").click(function (e){
					...
					$contents.find(".createPopover").prepend(PopoverElement).removeClass("createPopover");
					checkPopovers();
				});
	...
	////// On Ready/Click functions  //////
		function progressBarReady(){
			...
			$('.pbLabelToggle a').click(function (e){
				...
				if(labelState == "off"){
					$contents.find('.pblabel').hide();
				} else {
					$contents.find('.pblabel').show();
				}
				$contents.find('#progressbar').removeClass("btmMargin");
				if($contents.find('.pbHeightSmall').length>0 || $contents.find('.pbHeightMicro').length>0){
					$contents.find('#progressbar').addClass("btmMargin");
				}
			});
...
///////////////////////////////
	...
	////// On Ready/Click functions  //////
		function sectionsReady(sectionArray, mceInstance){
			...
			$("#sectionsButtons").append(' <a class="btn btn-mini remove-empty" href="#" data-tooltip="left"\
		...
		// If the checked section exists, move it, if not add it
			function checkSection(sectionName, sectionArray){
				...
				if($contents.find("."+sectionName).length>0){
					...
					$contents.find("#template-content").append(sectionArray[sectionName]);
				}
			}
							...
							// Check sections against default array
								if($.inArray(myValue, sectionArray) !== -1){
									// Not sure what is matching here at the moment, it should have been what wasn't in the default list
										...
										if($(this).attr("id")=="quickCheckOne"){
											var qcClass = "quickCheckOneContent";
											var qcTitle = "Quick Check 1";
										} else {
											var qcClass = "quickCheckTwoContent";
											var qcTitle = "Quick Check 2";
										}
	...
	////// Custom Tools Accordion tab setup  //////
		function studentVerification(){
			var addAccordionSection = '<h3>\
				...
				<a href="#" class="icon-add addVerification">Add</a> | \
				<a href="#" class="icon-end remove removeVerification">Remove</a><br>\
				<a href="#" class="icon-check insertStudentName">Insert Student Name</a><br>\
				</div>';
			$("#custom-tools-accordion").append(addAccordionSection);
			// $(".removeVerification").click( function (e){
			// 	e.preventDefault();
			// 	$(iframeID).contents().find('#studentVerification').remove();
			// });
	...
	////// On Ready/Click functions  //////
		function studentVerificationReady(){
			$(".addVerification").click( function (e){
				e.preventDefault();
				if($(iframeID).contents().find('#verficationSuccess').length>0){
				} else {
					var confirmationText = '<div id="verificationSuccess"><p>Enter text to be displayed to the student when submitted successfully</p></div>';
					$(iframeID).contents().find('body').prepend(confirmationText);
				}
				if($(iframeID).contents().find('#studentVerification').length>0){
					alert("Student Verfication section already exists. Edit the content to match your needs.");
				} else {
					var verificationText = '<div id="studentVerification"><p>I, <span class="studentName">Student Name Inserted Here</span>, acknowledge that I have read and understand ...</p></div>';
					$(iframeID).contents().find('body').prepend(verificationText);
				}
			...
			var addAccordionSection = '<h3>\
				...
				<div>\
					...
					<a class="help" data-tooltip=\'{"tooltipClass":"popover right", "position":"right"}\'\
					  ...
					  <div class=\'popover-content\'><p>Use this if you will be creating a custom stylesheet to override the default style.</p><p>This will look for a <em>style.css</em> file in a <em>global/css</em> folder in the course root folder.</p></div>">\
			...
			(function() {
				...
				$(".template-theme").click(function (e){
					...
					// add the class for the selected template to the template-wrapper
						...
						$contents.find("#template-wrapper").removeClass().addClass(templateClass);
							themeElements(templateClass);
						if($contents.find("#template-wrapper").hasClass(templateClass)){
				...
				$(".removeBannerLeft").click(function (e){
					e.preventDefault();
					$contents.find("#banner-left").remove();
					$contents.find("#module-page-banner h2").attr("style", "padding-left: 10px;")
				});
				customCSSCheck();
				currentThemeCheck();
			})();
	...
	////// Supporting functions  //////
		...
		// If template is not there add it, will also remove and change old template
			function templateCheck(){
				...
				if($contents.find("#template-wrapper").length>0){
					var currentTheme = $contents.find("#template-wrapper").attr("class");
		...
		// Ensure that necessary sections exist for a given style, will also fix broken banner
			function themeElements(templateClass){
				...
				if($contents.find('.mod-text').length>0 && $contents.find('.mod-text').text() !== ""){
					var modText = $contents.find('.mod-text').text();
				} else if ($contents.find('#page-left').length>0) {
					// Look for old template layout
					var modText = $contents.find('#page-left').text();
					// console.log(modText);
				} else {
					var modText = "Module";
				}
				// Look to see if title exists and grab value
				if($contents.find('#banner-right').length>0 && $contents.find('#banner-right').text() !== ""){
					var modTitle = $contents.find('#banner-right').text()
				} else if ($contents.find('#page-right').length>0) {
					var modTitle = $contents.find('#page-right').text();
				} else {
					var modTitle = "Title";
				}
				...
				if($contents.find('#module-page-banner').length>0){
					$contents.find('#module-page-banner').html(templateBanner);
				} else {
					$contents.find('#template-wrapper').prepend('<div id="module-page-banner">'+templateBanner+'</div>');
				}
				...
				if($.inArray(templateClass, bottomBannerThemeArray) == -1){
					...
					if($contents.find('#banner-bottom').length>0){
						$contents.find('#banner-bottom').hide();
					}
				// in the array, add or show the banner
				} else {
					if($contents.find('#banner-bottom').length>0){
						$contents.find('#banner-bottom').show();
					} else {
						$contents.find('#module-page-banner').after(bottomBanner);
					}
				}
			}
